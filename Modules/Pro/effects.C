...
/*
LOCAL void not_implemented(struct automaton *a, struct channel *ch)
	{
...
LOCAL void do_slide(struct channel *ch)
   {
   ch->pitch += ch->slide;
...
LOCAL void set_upslide(struct channel *ch, struct event *e)
   {
   ch->adjust = do_slide;
   if (e->parameters)
...
LOCAL void set_downslide(struct channel *ch, struct event *e)
   {
   ch->adjust = do_slide;
   if (e->parameters)
      ch->slide = -e->parameters;
...
LOCAL int sinusoid_value(struct sinusoid *s)
	{
	s->offset += s->rate;
	s->offset &= 63;
...
LOCAL void set_sinusoid(struct event *e, struct sinusoid *s)
	{
   if (HI(e->parameters))
...
LOCAL void do_vibrato(struct channel *ch)
   {
...
LOCAL void do_tremolo(struct channel *ch)
	{
	set_temp_volume(ch, ch->volume + sinusoid_value(&(ch->trem))/128);
	}

LOCAL void set_tremolo(struct channel *ch, struct event *e)
	{
 ...
 */
LOCAL void do_arpeggio(struct channel *ch)
   {
...
LOCAL void set_arpeggio(struct channel *ch, struct event *e)
   {
 ...
 */
LOCAL void do_slidevol(struct channel *ch)
   {
 ...
 */
LOCAL void parse_slidevol(struct channel *ch, int para)
   {
   if (HI(para))
...
LOCAL void set_slidevol(struct channel *ch, struct event *e)
   {
 ...
 */
LOCAL void do_portamento(struct channel *ch)
   {
	if (ch->pitchgoal)
		{
		...
		if (ch->funk_glissando)
			set_temp_pitch(ch, round_pitch(ch->pitch, ch->finetune));
		else
			set_temp_pitch(ch, ch->pitch);
		}
   }
 ...
 */
LOCAL void set_portamento(struct channel *ch, pitch pitch, struct event *e)
   {
   if (e->parameters)
...
LOCAL void do_portaslide(struct channel *ch)
   {
	do_portamento(ch);
   do_slidevol(ch);
   }

LOCAL void set_portaslide(struct channel *ch, pitch pitch, struct event *e)
   {
   if (pitch)
      ch->pitchgoal = pitch;
   parse_slidevol(ch, e->parameters);
...
LOCAL void do_vibratoslide(struct channel *ch)
   {
   do_vibrato(ch);
   do_slidevol(ch);
   }

LOCAL void set_vibratoslide(struct channel *ch, struct event *e)
   {
 ...
 */
LOCAL void set_speed(struct automaton *a, struct event *e)
   {
   if (e->parameters >= 32 && get_pref_scalar(PREF_SPEEDMODE) != OLD_SPEEDMODE)
      {
...
LOCAL void set_st_speed(struct automaton *a, struct event *e)
	{
	a->new_speed = e->parameters;
	a->do_stuff |= SET_SPEED;
	}

LOCAL void set_skip(struct automaton *a, struct event *e)
   {
...
LOCAL void set_fastskip(struct automaton *a, struct event *e)
   {
 ...
 * off the start.
 */
LOCAL void set_offset(struct channel *ch, struct event *e)
   {
	if (e->parameters)
 ...
 */
LOCAL void set_volume(struct channel *ch, struct event *e)
   {
 ...
 */
LOCAL void do_retrig(struct channel *ch)
   {
   if (--ch->current <= 0)
      {
		start_note(ch);
...
LOCAL void set_retrig(struct channel *ch, struct event *e)
   {
 ...
 */
LOCAL void do_latestart(struct channel *ch)
   {
   if (--ch->current <= 0)
      {
...
LOCAL void set_late_start(struct channel *ch, struct event *e)
   {
 ...
 */
LOCAL void do_cut(struct channel *ch)
   {
   if (ch->retrig)
      {
      if (--ch->retrig == 0)
...
LOCAL void set_note_cut(struct channel *ch, struct event *e)
   {
   ch->retrig = e->parameters;
   ch->adjust = do_cut;
   }
...
LOCAL void set_smooth_up(struct channel *ch, struct event *e)
   {
...
LOCAL void set_smooth_down(struct channel *ch, struct event *e)
   {
   ch->pitch -= e->parameters;
   ch->pitch = MIN(ch->pitch, MAX_PITCH);
   ch->pitch = MAX(ch->pitch, MIN_PITCH);
...
LOCAL void set_change_finetune(struct channel *ch, struct event *e)
   {
...
LOCAL void set_loop(struct channel *ch, struct automaton *a, struct event *e)
   {
...
LOCAL void set_smooth_upvolume(struct channel *ch, struct event *e)
   {
   set_current_volume(ch, ch->volume + e->parameters);
   }

LOCAL void set_smooth_downvolume(struct channel *ch, struct event *e)
   {
   set_current_volume(ch, ch->volume - e->parameters);
   }


LOCAL void set_delay_pattern(struct automaton *a, struct event *e)
   {
...
LOCAL void set_gliss_ctrl(struct channel *ch, struct event *e)
	{
	if (e->parameters)
		{
...
LOCAL void set_sine_wave(struct event *e, struct sinusoid *s)
	{
...
LOCAL void set_tremolo_wave(struct channel *ch, struct event *e)
	{
	set_sine_wave(e, &(ch->trem));
	}

LOCAL void do_invert(struct channel *ch)
	{
...
LOCAL void set_invert_loop(struct channel *ch, struct event *e)
	{
...
void init_effects(struct st_effect table[])
   {
   int i;
